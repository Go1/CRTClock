...
### 8ビットモード + CRTエフェクト
レトロな8ビット風デザインにCRTモニター効果を組み合わせた、ノスタルジックな表示です。

![8ビットモード](docs/images/8bit-crt-mode.png)
...
- **マテリアル**: モダンなマテリアルデザイン
- **8ビット**: レトロな8ビット風デザイン（Sixtyfourフォント）

#### フォントファミリー
- **モノスペース**: 等幅フォント（デフォルト）
- **サンセリフ**: モダンなゴシック体
- **セリフ**: クラシックな明朝体
- **ピクセル**: Sixtyfour 8ビット風フォント
...
#### 動的強度制御
- **CSS変数**: `--crt-opacity` 変数でエフェクト強度を統一制御
- **calc()関数**: 各エフェクトの透明度や強度を動的に計算
- **リアルタイム調整**: スライダー操作と同時にエフェクトが変化
...
### v2.5.0 - フォントサイズスケール調整機能
- **🆕 フォントサイズスケール**: 50%〜150%の範囲で細かく調整可能
- **🆕 フィットボタン**: ワンクリックで100%（画面フィット）に戻す機能
- **🔄 設定簡素化**: 従来の4段階フォントサイズ選択を削除し、より直感的なスケール調整に統一
- **⚡ リアルタイム調整**: スライダー操作と同時にサイズが変化
- **💾 設定移行**: 既存設定からの自動移行機能
- **📱 画面比率制御**: 画面幅に対する時計の実効部分の比率を自由に調整
